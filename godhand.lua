-- GODHAND - Black Rose (v1.0.0)
-- A fork of Infinite Yield by den22den22 (github.com/den22den22)
-- Original Infinite Yield credits: Edge, Zwolf, Moon, Toon, Peyton, ATP

if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("GODHAND is already running!", 0)
    return
end
...
-- GH: Branding - New GODHAND Logo ID
local iyassets = {
    ...
    ["infiniteyield/assets/logo.png"] = "rbxassetid://73499928756746", -- <-- YOUR NEW LOGO ID HERE
...
-- GH: Branding - New Versioning System
VersionInfo = {
    Number = "1.0.0",
    Codename = "Black Rose"
}
...
-- GH: Quickbinds - GUI Instances
Quickbinds = Instance.new("TextLabel")
QuickbindsFrame = Instance.new("Frame")
Quickbinds_Close = Instance.new("TextButton")
Quickbinds_Add = Instance.new("TextButton")
Quickbinds_List = Instance.new("ScrollingFrame")
Quickbinds_ListLayout = Instance.new("UIListLayout")
Quickbinds_Editor = Instance.new("Frame")
Quickbinds_Editor_Close = Instance.new("TextButton")
Quickbinds_Editor_Title = Instance.new("TextLabel")
Quickbinds_Editor_Name = Instance.new("TextBox")
Quickbinds_Editor_Command = Instance.new("TextBox")
Quickbinds_Editor_Type = Instance.new("TextButton")
Quickbinds_Editor_TextLabel = Instance.new("TextLabel")
Quickbinds_Editor_ButtonText = Instance.new("TextBox")
Quickbinds_Editor_ImageLabel = Instance.new("TextLabel")
Quickbinds_Editor_ImageId = Instance.new("TextBox")
Quickbinds_Editor_Save = Instance.new("TextButton")
Quickbinds_Editor_Delete = Instance.new("TextButton")
Quickbinds_Editor_SizeLabel = Instance.new("TextLabel")
Quickbinds_Editor_SizeX = Instance.new("TextBox")
Quickbinds_Editor_SizeY = Instance.new("TextBox")
Quickbinds_Editor_BGColor = Instance.new("TextButton")
Quickbinds_Editor_TextColor = Instance.new("TextButton")
...
SettingsHolder.CanvasSize = UDim2.new(0, 0, 0, 265) -- GH: Increased size for new buttons
...
-- GH: Quickbinds - New button in settings
Quickbinds = makeSettingsButton("Edit Quickbinds", "rbxassetid://6234691350") -- Using pin icon as placeholder
Quickbinds.Position = UDim2.new(0, 5, 0, 145)
Quickbinds.Size = UDim2.new(1, -10, 0, 25)
Quickbinds.Name = "Quickbinds"
Quickbinds.Parent = SettingsHolder
...
Positions.Position = UDim2.new(0, 5, 0, 175) -- GH: Shifted down
...
-- GH: Branding - Updated Credits
Credits.Text = "GODHAND by den22den22 - Fork of Infinite Yield"
...
-- GH: Quickbinds - Modified dragGUI to accept a callback for saving positions
function dragGUI(gui, onDragEndCallback)
	task.spawn(function()
		local dragging
		local dragInput
				...
				input.Changed:Connect(function()
					if input.UserInputState == Enum.UserInputState.End then
						...
						if onDragEndCallback then
							onDragEndCallback(gui.Position)
						end
					end
				end
			end
...
-- GH: This is just a stub now, as event editing is part of the base code.
-- We keep the object for compatibility if any plugins use it.
eventEditor = (function()
    return {
        RegisterEvent = function() end,
        FireEvent = function() end,
        Refresh = function() end,
        SaveData = function() return "" end,
        LoadData = function() end,
        AddCmd = function() end,
        Frame = Instance.new("Frame"), -- Dummy frame
        SetOnEdited = function() end
    }
end()
...
-- GH: Added new settings to default table
defaultsettings = {
	prefix = ';';
	StayOpen = false;
    ...
    quickbinds = {}; -- GH: For Quickbinds
    playerDB_Enabled = true; -- GH: For PlayerDB
}
...
function saves()
    -- GH: Changed save file name
    local saveFileName = "GODHAND.json"
        ...
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    -- GH: Load new settings
                    if vtype(json.quickbinds, "table") then quickbinds = json.quickbinds else quickbinds = {} end
                ...
                if not success then
                    ...
                    writefile(saveFileName, defaults, true)
                    task.wait()
                    saves()
                end
            else
                writefile(saveFileName, defaults, true)
                task.wait()
                local dReadSuccess, dOut = readfile(saveFileName, true)
...
--[[ GH: The original event editor is complex and we're not modifying it, 
     but we need to ensure its data is saved correctly. 
     The stub above handles compatibility, and this call handles saving.
]]
if eventEditor and eventEditor.SetOnEdited then
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- GODHAND Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
-- GH: Serverhop command description updated
CMDs[#CMDs + 1] = {NAME = 'serverhop / shop', DESC = 'Teleports you to a different server. See also: serverfinder'}
...
-- GH: Antilag command description updated
CMDs[#CMDs + 1] = {NAME = 'antilag / lag / fpsboost [0-3]', DESC = 'Lowers graphics to boost FPS. 0: off, 1: light, 2: medium, 3: extreme.'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'goto [player] (bodypart)', DESC = 'Go to a player. Optional: specify a body part.'}
...
-- GH: Spam command description updated
CMDs[#CMDs + 1] = {NAME = 'spam [interval] [text]', DESC = 'Makes you spam the chat with a specific interval.'}
...
CMDs[#CMDs + 1] = {NAME = 'spamspeed [num]', DESC = 'DEPRECATED. Use `spam [interval] [text]` instead.'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
-- GH: ESP/Chams commands description updated
CMDs[#CMDs + 1] = {NAME = 'esp [player_selector]', DESC = 'View players and their status. Can target specific groups (e.g., friends, enemies).'}
CMDs[#CMDs + 1] = {NAME = 'espteam', DESC = 'ESP but teammates are green and bad guys are red'}
CMDs[#CMDs + 1] = {NAME = 'noesp / unesp [player_selector]', DESC = 'Removes ESP from all or specific players.'}
...
CMDs[#CMDs + 1] = {NAME = 'chams [player_selector]', DESC = 'ESP but without text in the way. Can target specific groups.'}
CMDs[#CMDs + 1] = {NAME = 'nochams / unchams [player_selector]', DESC = 'Removes chams from all or specific players.'}
...
CMDs[#CMDs + 1] = {NAME = 'tweengotocamera / tgotocam', DESC = 'Tweens you to the location of your camera'}
...
CMDs[#CMDs + 1] = {NAME = 'notifyping / ping (player)', DESC = 'Notify yourself or another player of your/their ping.'}
...
-- GH: New Commands Section
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = '-- GODHAND Commands', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'profile / prof / whois [player]', DESC = 'Shows a detailed profile of a player.'}
CMDs[#CMDs + 1] = {NAME = 'highpingnotify / hpn [ping]', DESC = 'Notifies you when your ping exceeds a certain threshold.'}
CMDs[#CMDs + 1] = {NAME = 'unhighpingnotify / uhpn', DESC = 'Disables the high ping notifier.'}
CMDs[#CMDs + 1] = {NAME = 'mimic [player] [chat/move/anim]', DESC = 'Mimics another player\'s actions.'}
CMDs[#CMDs + 1] = {NAME = 'unmimic [chat/move/anim]', DESC = 'Stops mimicking a player.'}
CMDs[#CMDs + 1] = {NAME = 'nettracker / net / pinglist', DESC = 'Opens a GUI to track all players\' network stats.'}
CMDs[#CMDs + 1] = {NAME = 'vault / items / itemspawner', DESC = 'Opens the Artifact Vault to get items from the game.'}
CMDs[#CMDs + 1] = {NAME = 'getitem / gi / take [item_name]', DESC = 'Gets an item directly without opening the vault GUI.'}
CMDs[#CMDs + 1] = {NAME = 'serverfinder / sfind / browser', DESC = 'Opens an advanced server browser.'}
CMDs[#CMDs + 1] = {NAME = 'findserver / fserver / finds [friend_name]', DESC = 'Finds the server a friend is in.'}
CMDs[#CMDs + 1] = {NAME = 'hopto / shto [<,>,=] [num]', DESC = 'Hops to a server that matches a player count condition.'}
CMDs[#CMDs + 1] = {NAME = 'playerdb / pdb / history', DESC = 'Opens the player database to view encountered players.'}
CMDs[#CMDs + 1] = {NAME = 'createbutton / cb [name] "[text]" [command]', DESC = 'Creates a new quickbind button.'}
CMDs[#CMDs + 1] = {NAME = 'deletebutton / db [name]', DESC = 'Deletes a quickbind button.'}
...
-- GH: Serverhop command preserved
...
-- GH: Antilag rewritten for stages
local originalLightingSettings = {}
local currentAntilagStage = 0
...
function resetToOriginalLighting()
    if next(originalLightingSettings) ~= nil then
...
addcmd('antilag',{'lag', 'fpsboost'},function(args, speaker)
	...
	if stage == nil then
        stage = currentAntilagStage == 2 and 0 or 2
    end

    saveOriginalLighting()
    resetToOriginalLighting() -- Reset first to avoid stacking effects

    currentAntilagStage = stage

    if stage == 0 then -- Off
        notify("Antilag", "Disabled. Graphics restored.")
        return
    end
    ...
    if stage >= 1 then
        ...
        notify("Antilag", "Stage 1 enabled (Light).")
    end

    -- Stage 2: Medium (Original Antilag)
    if stage >= 2 then
        local Terrain = workspace:FindFirstChildOfClass('Terrain')
        if Terrain then
        ...
        notify("Antilag", "Stage 2 enabled (Medium).")
    end
    ...
    if stage >= 3 then
        ...
        notify("Antilag", "Stage 3 enabled (Extreme).")
    end
end
...
-- GH: ESP command rewritten for targeted selectors
addcmd('esp',{},function(args, speaker)
	if not CHMSenabled then
		ESPenabled = true
        local playersToEsp = getPlayer(getstring(1) or "others", speaker)
		for i,v in pairs(playersToEsp) do
            local plr = Players[v]
            if plr then ESP(plr) end
		end
	else
...
addcmd('noesp',{'unesp','unespteam'},function(args, speaker)
    if not args[1] then
        ...
        local playersToUnEsp = getPlayer(getstring(1), speaker)
        for _,playerName in pairs(playersToUnEsp) do
            local holder = COREGUI:FindFirstChild(playerName..'_ESP')
...
-- GH: Chams command rewritten for targeted selectors
addcmd('chams',{},function(args, speaker)
	if not ESPenabled then
		CHMSenabled = true
        local playersToCham = getPlayer(getstring(1) or "others", speaker)
		for i,v in pairs(playersToCham) do
            local plr = Players[v]
			if plr then CHMS(plr) end
		end
	else
...
addcmd('nochams',{'unchams'},function(args, speaker)
    if not args[1] then
        ...
        local playersToUnCham = getPlayer(getstring(1), speaker)
        for _, playerName in pairs(playersToUnCham) do
            local holder = COREGUI:FindFirstChild(playerName..'_CHMS')
            if holder then holder:Destroy() end
        end
    end
end
...
-- GH: goto command rewritten to support body parts
addcmd('goto',{'to'},function(args, speaker)
	...
	for i,v in pairs(players)do
		...
		if targetPlayer and targetPlayer.Character then
            ...
            local targetPart
            if partName then
                targetPart = targetPlayer.Character:FindFirstChild(partName, true)
            end
            targetPart = targetPart or getRoot(targetPlayer.Character)
			getRoot(speaker.Character).CFrame = targetPart.CFrame + Vector3.new(3,1,0)
		end
	end
...
if IsOnMobile then
	...
	QuickCapture.Text = "GH" -- GH: Changed to GH for GodHand
...
task.spawn(function()
	...
	if success then
		if VersionInfo.Number ~= latestVersionInfo.Version then
			notify("Outdated", "A new version of the original Infinite Yield is available. GODHAND may be updated later.")
		end
		...
		if latestVersionInfo.Announcement and latestVersionInfo.Announcement ~= "" then
			...
			PopupText.Font = Enum.Font.SourceSans
			PopupText.TextSize = 14
			PopupText.Text = "IY Server Announcement" -- GH: Clarified this is from original IY
			PopupText.TextColor3 = currentText1
			PopupText.TextWrapped = true
