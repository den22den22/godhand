...
local CHAMS_SelectorString = nil -- Будет хранить правило для Chams
...
-- xylex & europa
local iyassets = {
    ["godhand/assets/bindsandplugins.png"] = "rbxassetid://5147695474",
    ["godhand/assets/close.png"] = "rbxassetid://5054663650",
    ["godhand/assets/editaliases.png"] = "rbxassetid://5147488658",
    ["godhand/assets/editkeybinds.png"] = "rbxassetid://129697930",
    ["godhand/assets/edittheme.png"] = "rbxassetid://4911962991",
    ["godhand/assets/editwaypoints.png"] = "rbxassetid://5147488592",
    ["godhand/assets/imgstudiopluginlogo.png"] = "rbxassetid://4113050383",
    ["godhand/assets/logo.png"] = "rbxassetid://73499928756746",
    ["godhand/assets/minimize.png"] = "rbxassetid://2406617031",
    ["godhand/assets/pin.png"] = "rbxassetid://6234691350",
    ["godhand/assets/reference.png"] = "rbxassetid://3523243755",
    ["godhand/assets/settings.png"] = "rbxassetid://1204397029"
}
...
if makefolder and isfolder and writefile and isfile then
    pcall(function() -- good executor trust
        local assets = "https://raw.githubusercontent.com/den22den22/godhand/main/"
        for _, folder in {"godhand", "godhand/assets"} do
        ...
        for path in iyassets do
            if not isfile(path) then
                writefile(path, game:HttpGet((path:gsub("godhand/", assets))))
            end
        end
        if IsOnMobile then writefile("godhand/assets/.nomedia") end
    end)
end
...
local qbEditorName = Instance.new("TextBox")
local qbEditorCommand = Instance.new("TextBox")
local qbEditorTypeToggle = Instance.new("TextButton")
local qbEditorTextLabel = Instance.new("TextLabel")
local qbEditorTextInput = Instance.new("TextBox")
local qbEditorImageLabel = Instance.new("TextLabel")
local qbEditorImageInput = Instance.new("TextBox")
local qbEditorSizeLabel = Instance.new("TextLabel")
local qbEditorWidth = Instance.new("TextBox")
local qbEditorHeight = Instance.new("TextBox")
local qbEditorSave = Instance.new("TextButton")
local qbAddNewButton = Instance.new("TextButton")
local qbCloseButton = Instance.new("TextButton")

-- Окно Базы Данных Игроков (PlayerDB)
PlayerDBFrame = Instance.new("Frame")
PlayerDBButton = nil -- Будет создано позже
local pdbList = Instance.new("ScrollingFrame")
local pdbSearchBox = Instance.new("TextBox")
local pdbDetails = Instance.new("Frame")
local pdbCloseButton = Instance.new("TextButton")

-- Окно Профиля Игрока
ProfileFrame = Instance.new("Frame")

-- Окно Поиска Серверов
ServerFinderFrame = Instance.new("Frame")

-- Окно Сетевого Трекера
NetTrackerFrame = Instance.new("Frame")
...
Title.TextSize = 18
Title.Text = "GODHAND - " .. VersionInfo.Codename .. " (v" .. VersionInfo.Number .. ")"
...
SettingsButton.Image = getcustomasset("godhand/assets/settings.png")
...
ReferenceButton.Image = getcustomasset("godhand/assets/reference.png")
...
ColorsButton = makeSettingsButton("Edit Theme",getcustomasset("godhand/assets/edittheme.png"))
ColorsButton.Position = UDim2.new(0, 5, 0, 55)
ColorsButton.Size = UDim2.new(1, -10, 0, 25)
ColorsButton.Name = "Colors"
ColorsButton.Parent = SettingsHolder

Keybinds = makeSettingsButton("Edit Keybinds",getcustomasset("godhand/assets/editkeybinds.png"))
Keybinds.Position = UDim2.new(0, 5, 0, 85)
Keybinds.Size = UDim2.new(1, -10, 0, 25)
Keybinds.Name = "Keybinds"
Keybinds.Parent = SettingsHolder

Aliases = makeSettingsButton("Edit Aliases",getcustomasset("godhand/assets/editaliases.png"))
...
Positions = makeSettingsButton("Edit/Goto Waypoints",getcustomasset("godhand/assets/editwaypoints.png"))
Positions.Position = UDim2.new(0, 5, 0, 145)
Positions.Size = UDim2.new(1, -10, 0, 25)
Positions.Name = "Waypoints"
Positions.Parent = SettingsHolder

EventBind = makeSettingsButton("Edit Event Binds",getcustomasset("godhand/assets/bindsandplugins.png"),759)
EventBind.Position = UDim2.new(0, 5, 0, 205)
EventBind.Size = UDim2.new(1, -10, 0, 25)
EventBind.Name = "EventBinds"
EventBind.Parent = SettingsHolder

Plugins = makeSettingsButton("Manage Plugins",getcustomasset("godhand/assets/bindsandplugins.png"),743)
...
table.insert(text1, HeaderLabel)
...
CloseImage.Image = getcustomasset("godhand/assets/close.png")
...
PinImage.ZIndex = 10
PinImage.Image = getcustomasset("godhand/assets/pin.png")
...
Logo.Image = getcustomasset("godhand/assets/logo.png")
...
Credits.Text = "GODHAND Fork by den22den22 // Original IY: Edge, Zwolf, Moon & others"
...
ExitImage_2.Image = getcustomasset("godhand/assets/close.png")
...
Img.Image = getcustomasset("godhand/assets/imgstudiopluginlogo.png")
Img.ZIndex = 10
...
ExitImage_3.Image = getcustomasset("godhand/assets/close.png")
...
ExitImage_5.Image = getcustomasset("godhand/assets/close.png")
...
ImageLabel.Image = getcustomasset("godhand/assets/minimize.png")
...
ImageLabel_2.Image = getcustomasset("godhand/assets/close.png")
...
function dragGUI(gui, callback) -- Добавляем callback
    task.spawn(function()
        local dragging
        local dragInput
        ...
        local function update(input)
            ...
            gui.Position = Position -- Убираем Tween для мгновенного отклика
        end
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End then
                        dragging = false
                        if callback then -- Если передан callback, вызываем его
                            callback(gui.Position)
                        end
                    end
                end)
            end
...
eventEditor = (function()
	...
	local main = create({
		...
		{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=getcustomasset("godhand/assets/close.png"),Parent={4},Position=UDim2.new(0,5,0,5),Size=UDim2.new(0,10,0,10),ZIndex=10,}},
		...
		{53,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=getcustomasset("godhand/assets/settings.png"),Parent={52},Position=UDim2.new(0,2,0,2),Size=UDim2.new(0,16,0,16),ZIndex=10,}},
	})
	main.Name = randomString()
        ...
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    if vtype(json.quickbinds, "table") then AllQuickbinds = json.quickbinds else AllQuickbinds = {} end
...
IndexContents = function(str,bool,cmdbar,Ianim)
	...
	for i,v in next, frame:GetChildren() do
		if v.Name == "CMD" then -- Работаем ТОЛЬКО с командами
			if bool then
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Movement', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Teleport', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Chat & Logs', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'World & Items', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Character', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Player Interaction', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Automation & Input', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Script Settings', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Miscellaneous', DESC = ''}
...
for i = 1, #CMDs do
	if CMDs[i].DESC == '' and CMDs[i].NAME ~= '' then -- Это заголовок
		local newHeader = Example:Clone()
		newHeader.Parent = CMDsF
		newHeader.Visible = true
		newHeader.LayoutOrder = i
		newHeader.Text = CMDs[i].NAME
		newHeader.Name = "Header"
		newHeader.Font = Enum.Font.SourceSansBold
		newHeader.TextColor3 = Color3.fromRGB(200, 200, 200)
		newHeader.TextSize = 18 -- УВЕЛИЧЕНО С 16 ДО 18
		newHeader.Size = UDim2.new(1, 0, 0, 22) -- УВЕЛИЧЕНО С 20 ДО 22
		newHeader.Selectable = false
		newHeader:SetAttribute("Title", nil)
		newHeader:SetAttribute("Desc", nil)
	elseif CMDs[i].DESC == '' and CMDs[i].NAME == '' then -- Это разделитель
        ...
        spacer.Visible = true
	else -- Это обычная команда
		local newcmd = Example:Clone()
		newcmd.Parent = CMDsF
		newcmd.Visible = false
		newcmd.LayoutOrder = i
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/den22den22/godhand/main/godhand.lua'))()")
	end
end)
...
-- ##################################################################
-- #          [+] GODHAND: ИСПРАВЛЕННЫЙ И НАДЕЖНЫЙ ANTILAG          #
-- ##################################################################

local antilagStage = 0 -- Отслеживает текущий уровень antilag
local antilagOriginalLighting = nil -- Хранит только оригинальные настройки освещения

-- Функция для восстановления графики. Простая и надежная.
local function unantilag()
    settings().Rendering.QualityLevel = Enum.QualityLevel.Automatic -- Возвращаем авто-качество

    -- Восстанавливаем освещение, если оно было сохранено
    if antilagOriginalLighting then
        for property, value in pairs(antilagOriginalLighting) do
            pcall(function() Lighting[property] = value end)
        end
        antilagOriginalLighting = nil -- Очищаем сохраненные данные
    end

    -- Возвращаем материалы к стандартному SmoothPlastic, как в оригинальном IY
    for _, v in ipairs(workspace:GetDescendants()) do
        if v:IsA("BasePart") and not v.Parent:FindFirstChildWhichIsA("Humanoid") then
            v.Material = Enum.Material.SmoothPlastic
            v.Reflectance = 0
        elseif v:IsA("ParticleEmitter") or v:IsA("Trail") then
            v.Lifetime = NumberRange.new(5) -- Возвращаем частицам жизнь
        end
    end
    ...
    notify("Antilag", "Отключен. Графика восстановлена до стандартных значений.")
end


addcmd('antilag', {'lag', 'fpsboost', 'lowgraphics'}, function(args, speaker)
    local stage = tonumber(args[1])
    if stage == nil then 
        antilagStage = (antilagStage >= 4 and 0 or antilagStage + 1) -- Цикличное переключение 0->1->2->3->4->0
    else
        antilagStage = math.clamp(stage, 0, 4)
    end
    
    -- Сохраняем оригинальные настройки освещения только один раз
    if antilagStage > 0 and not antilagOriginalLighting then
        antilagOriginalLighting = {
    ...
    -- СБРОС НАСТРОЕК (antilag 0)
    if antilagStage == 0 then
        unantilag()
        -- Применяем X-ray поверх восстановленных настроек, если он включен
        if xrayEnabled then applyXrayEffect(true) end 
        return
    end

    -- ПРИМЕНЕНИЕ НАСТРОЕК ANTILAG
    -- STAGE 1 (Базовый)
    ...
    -- STAGE 2 (Убираем текстуры и эффекты)
    if antilagStage >= 2 then
        local Terrain = workspace:FindFirstChildOfClass('Terrain')
        if Terrain then
        ...
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("BasePart") and not v.Parent:FindFirstChildWhichIsA("Humanoid") then
                 v.Material = Enum.Material.Plastic -- Меняем материал на простой пластик
                 v.Reflectance = 0
            end
            if v:IsA("Decal") or v:IsA("Texture") then v.Transparency = 1 end -- Делаем невидимыми, а не удаляем
            if v:IsA("ParticleEmitter") or v:IsA("Trail") then v.Lifetime = NumberRange.new(0) end
        end
    end
    
    -- STAGE 3 (Упрощаем освещение и воду)
    if antilagStage >= 3 then
         local Terrain = workspace:FindFirstChildOfClass('Terrain')
         if Terrain then Terrain.WaterTransparency = 1 end
         for _, light in ipairs(Lighting:GetChildren()) do
            if light:IsA("Light") then light.Shadows = false end
         end
    end
    
    -- STAGE 4 (Максимальное упрощение)
    if antilagStage >= 4 then
    ...
    notify("Antilag", "Установлен уровень " .. antilagStage)
    -- Применяем X-ray поверх, если он включен
    if xrayEnabled then applyXrayEffect(true) end
end)
...
addcmd('esp',{},function(args, speaker)
	...
	execCmd("noesp nonotify") -- Сначала очищаем старый ESP
	
	local selector = args[1] or "others"
	ESP_SelectorString = selector -- <<-- САМОЕ ВАЖНОЕ: Сохраняем правило!
	
	local playersToTarget = getPlayer(selector, speaker)
	for _, playerName in ipairs(playersToTarget) do
		local playerObject = Players:FindFirstChild(playerName)
		if playerObject then
			ESP(playerObject)
		end
	end
	notify("Targeted ESP", "Enabled for selector: " .. selector)
end)
...
-- ЗАМЕНИТЕ addcmd('chams',...)
addcmd('chams',{},function(args, speaker)
	if ESP_SelectorString then
		return notify('Chams','Disable ESP (noesp) before using chams')
	end

	execCmd("nochams nonotify") -- Сначала очищаем старые Chams
	
	local selector = args[1] or "others"
	CHAMS_SelectorString = selector -- <<-- САМОЕ ВАЖНОЕ: Сохраняем правило!
	...
	notify("Targeted Chams", "Enabled for selector: " .. selector)
end)

addcmd('nochams',{'unchams'},function(args, speaker)
	CHAMS_SelectorString = nil
	CHMSenabled = false
	for i,c in pairs(COREGUI:GetChildren()) do
...
addcmd('goto',{'to'},function(args, speaker)
    local players = getPlayer(args[1], speaker)
    local bodyPart = args[2] and string.lower(args[2])
    for i,v in pairs(players)do
        local targetChar = Players[v].Character
        if targetChar then
            if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
                speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
                wait(.1)
            ...
            local targetPart = getRoot(targetChar) -- По умолчанию
            if bodyPart then
                for _, part in ipairs(targetChar:GetChildren()) do
                    if part:IsA("BasePart") and string.lower(part.Name):find(bodyPart) then
                        targetPart = part
                        break
                    end
                end
            ...
            if targetPart then
                getRoot(speaker.Character).CFrame = targetPart.CFrame + Vector3.new(0, 3, 0)
            end
        end
    end
...
-- [+] GODHAND: ПЕРЕРАБОТАННЫЙ БЛОК X-RAY ДЛЯ СОВМЕСТИМОСТИ С ANTILAG

local xrayEnabled = false -- Эта переменная будет хранить состояние X-Ray

-- Основная функция, которая применяет или убирает эффект прозрачности
function applyXrayEffect(state)
    local transparency = state and 0.7 or 0 -- 0.7 если включен, 0 если выключен
    for _, v in pairs(workspace:GetDescendants()) do
...
addcmd("xray", {}, function(args, speaker)
    if xrayEnabled then return end -- Если уже включен, ничего не делаем
    xrayEnabled = true
...
addcmd("unxray", {"noxray"}, function(args, speaker)
    if not xrayEnabled then return end -- Если уже выключен, ничего не делаем
    xrayEnabled = false
...
addcmd("togglexray", {}, function(args, speaker)
    ...
    applyXrayEffect(xrayEnabled)
end)

-- Команда для зацикленного включения
addcmd("loopxray", {}, function(args, speaker)
    pcall(function() xrayLoop:Disconnect() end) -- Отключаем старый цикл, если он был
    xrayEnabled = true -- Устанавливаем состояние
    xrayLoop = RunService.RenderStepped:Connect(function()
        applyXrayEffect(true) -- Постоянно применяем эффект
    end)
end)

-- Команда для отключения цикла
addcmd("unloopxray", {}, function(args, speaker)
    pcall(function() xrayLoop:Disconnect() end) -- Отключаем цикл
    if xrayEnabled then
        xrayEnabled = false
        applyXrayEffect(false) -- Убираем эффект после отключения
    end
end)
...
-- ЗАМЕНИТЕ ВЕСЬ БЛОК Players.PlayerAdded
Players.PlayerAdded:Connect(function(plr)
	...
	-- GODHAND: Новая логика для ESP/Chams при входе нового игрока
	task.spawn(function()
		if ESP_SelectorString then
			local playersToTarget = getPlayer(ESP_SelectorString, Players.LocalPlayer)
			if table.find(playersToTarget, plr.Name) then
				local char = plr.Character or plr.CharacterAdded:Wait()
				if char then ESP(plr) end
			end
		end
		
		if CHAMS_SelectorString then
			local playersToTarget = getPlayer(CHAMS_SelectorString, Players.LocalPlayer)
			if table.find(playersToTarget, plr.Name) then
				local char = plr.Character or plr.CharacterAdded:Wait()
				if char then CHMS(plr) end
			end
		end
	end)
end)
...
task.spawn(function()
	wait()
	-- Анимация появления строки с авторами. Можно оставить как есть.
	Credits:TweenPosition(UDim2.new(0, 0, 0.9, 0), "Out", "Quart", 0.2)
	
	-- Анимация "приближения" лого. Увеличиваем длительность с 0.3 до 0.8 секунды.
	Logo:TweenSizeAndPosition(UDim2.new(0, 175, 0, 175), UDim2.new(0, 37, 0, 45), "Out", "Quart", 0.8) -- ИЗМЕНЕНО
	
	-- Пауза, пока логотип виден. Увеличиваем с 1 до 2.5 секунд.
	wait(2.5) -- ИЗМЕНЕНО
	
	-- Анимация исчезновения. Увеличиваем длительность с ~1.7 до 3 секунд и меняем стиль на более плавный (Quart).
	local OutInfo = TweenInfo.new(3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out, 0, false, 0) -- ИЗМЕНЕНО
	...
	-- Убираем анимацию исчезновения Credits, чтобы она не мешала.
	-- Credits:TweenPosition(UDim2.new(0, 0, 0.9, 30), "Out", "Quart", 0.2)
	
	-- Ждем, пока анимация исчезновения почти закончится.
	wait(2.8) -- ИЗМЕНЕНО
	
	-- Уничтожаем объекты и сворачиваем окно.
	Logo:Destroy()
	Credits:Destroy()
...
-- ##################################################################
-- #                     GODHAND - НОВЫЙ ФУНКЦИОНАЛ                 #
-- ##################################################################

--[[
    Модуль: Quickbinds
    Описание: Позволяет создавать кастомные кнопки на экране.
]]
local QuickbindButtons = {} -- Таблица для хранения объектов кнопок

function UpdateQuickbindSave(id, position, size)
    for i, data in ipairs(AllQuickbinds) do
        if data.ID == id then
            if position then
                AllQuickbinds[i].Position = {position.X.Scale, position.X.Offset, position.Y.Scale, position.Y.Offset}
            end
            if size then
                 AllQuickbinds[i].Size = {size.X.Offset, size.Y.Offset}
            end
            updatesaves()
            break
        end
...
-- [+] GODHAND: ЗАМЕНИТЕ СУЩЕСТВУЮЩУЮ ФУНКЦИЮ 'CreateQuickbindButton' НА ЭТУ ВЕРСИЮ
function CreateQuickbindButton(data)
    local button
    -- Логика создания Text/Image кнопки остается прежней
    if data.Type == "Image" then
        button = Instance.new("ImageButton")
        button.Image = "rbxassetid://" .. data.Customization.ImageId
        button.BackgroundColor3 = Color3.fromRGB(unpack(data.Customization.ImageColor))
    else -- Text
        button = Instance.new("TextButton")
        button.Text = data.Customization.ButtonText
        button.BackgroundColor3 = Color3.fromRGB(unpack(data.Customization.BackgroundColor))
        button.TextColor3 = Color3.fromRGB(unpack(data.Customization.TextColor))
        button.Font = Enum.Font.SourceSansBold
        button.TextSize = 14
    end

    button.Name = "Quickbind_" .. data.ID
    button.Parent = ScaledHolder
    button.Size = UDim2.new(0, data.Size[1], 0, data.Size[2])
    button.Position = UDim2.new(data.Position[1], data.Position[2], data.Position[3], data.Position[4])
    button.ZIndex = 20
    
    button:SetAttribute("ID", data.ID)

    -- Новая логика для обработки нажатий
    if data.IsToggle then
        -- Это кнопка-переключатель
        button:SetAttribute("ToggleState", false) -- Начальное состояние "выключено"
        
        button.MouseButton1Click:Connect(function()
            local currentState = button:GetAttribute("ToggleState")
            local commandToExecute
            
            if not currentState then -- Была выключена, теперь включаем
                commandToExecute = data.Command.On
                button.BackgroundColor3 = Color3.fromRGB(180, 60, 60) -- Цвет активного состояния
            else -- Была включена, теперь выключаем
                commandToExecute = data.Command.Off
                button.BackgroundColor3 = Color3.fromRGB(unpack(data.Customization.BackgroundColor)) -- Возвращаем исходный цвет
            end
            
            execCmd(commandToExecute, Players.LocalPlayer, false)
            button:SetAttribute("ToggleState", not currentState) -- Инвертируем состояние
        end)
    else
        -- Это обычная кнопка (старая логика)
        button:SetAttribute("Command", data.Command)
        button.MouseButton1Click:Connect(function()
            execCmd(button:GetAttribute("Command"), Players.LocalPlayer, false)
        end)
    end
    
    dragGUI(button, function(newPos)
        UpdateQuickbindSave(data.ID, newPos, nil)
    end)
    
    table.insert(QuickbindButtons, button)
end

function LoadQuickbinds()
    for _, button in ipairs(QuickbindButtons) do
        button:Destroy()
    end
    QuickbindButtons = {}
    
    for _, data in ipairs(AllQuickbinds) do
        CreateQuickbindButton(data)
    end
end

-- [+] GODHAND: ЗАМЕНИТЕ СУЩЕСТВУЮЩУЮ КОМАНДУ 'createbutton' НА ЭТУ ВЕРСИЮ
addcmd('createbutton', {'cb', 'newbutton'}, function(args, speaker)
    if #args < 2 then 
        return notify("Quickbinds", "Использование:\n- createbutton [имя] [команда]\n- createbutton [имя] [команда_ВКЛ],[команда_ВЫКЛ]") 
    end
    
    local name = args[1]
    local commandString = table.concat(args, " ", 2)
    
    -- Используем существующую функцию splitString для разделения команд
    local commands = splitString(commandString, ",")
    
    local isToggle = #commands > 1
    
    local newData = {
        ID = "qb_" .. tostring(tick()),
        Type = "Text",
        Name = name,
        IsToggle = isToggle, -- Новое поле для определения типа кнопки
        Position = {0.5, -50, 0.5, -15},
        Size = {100, 30},
        Customization = {
            ButtonText = name,
            BackgroundColor = {46, 46, 47}, -- Цвет по умолчанию (неактивный)
            TextColor = {255, 255, 255}
        }
    }

    if isToggle then
        -- Сохраняем обе команды для переключателя
        newData.Command = { On = commands[1], Off = commands[2] }
    else
        -- Сохраняем одну команду для обычной кнопки
        newData.Command = commandString
    end

    table.insert(AllQuickbinds, newData)
    CreateQuickbindButton(newData) -- Эта функция будет обновлена далее
    updatesaves()
    notify("Quickbinds", "Кнопка '" .. name .. "' создана (" .. (isToggle and "Переключатель" or "Обычная") .. ").")
end)

-- [+] GODHAND: НОВАЯ КОМАНДА ДЛЯ УДАЛЕНИЯ КНОПОК
addcmd('deletebutton', {'delbutton', 'db'}, function(args, speaker)
    if #args < 1 then return notify("Quickbinds", "Использование: deletebutton [имя кнопки]") end
    
    local nameToDelete = table.concat(args, " ")
    local found = false

    -- Итерируем в обратном порядке для безопасного удаления
    for i = #AllQuickbinds, 1, -1 do
        if AllQuickbinds[i].Name == nameToDelete then
            local idToDelete = AllQuickbinds[i].ID
            -- Удаляем данные о кнопке
            table.remove(AllQuickbinds, i)
            
            -- Находим и уничтожаем сам объект GUI
            for j = #QuickbindButtons, 1, -1 do
                if QuickbindButtons[j]:GetAttribute("ID") == idToDelete then
                    QuickbindButtons[j]:Destroy()
                    table.remove(QuickbindButtons, j)
                    break 
                end
            end
    ...
    if found then
        updatesaves()
        notify("Quickbinds", "Кнопка '" .. nameToDelete .. "' удалена.")
    else
        notify("Quickbinds", "Кнопка '" .. nameToDelete .. "' не найдена.")
    end
end)

--[[
    Модуль: База Данных Игроков
    Описание: Собирает информацию о встреченных игроках.
]]
function DB_Save()
    if PlayerDB_Enabled then
        writefile("godhand_playerdb.json", HttpService:JSONEncode(PlayerDatabase))
    end
end

function DB_Load()
    if readfile and isfile and isfile("godhand_playerdb.json") then
        local success, data = pcall(function()
            return HttpService:JSONDecode(readfile("godhand_playerdb.json"))
        end)
...
function DB_AddEncounter(player)
    ...
    if not PlayerDatabase[userId] then
    ...
    -- Обновляем имена на случай смены
    PlayerDatabase[userId].Username = player.Name
    PlayerDatabase[userId].DisplayName = player.DisplayName
    
    if not PlayerDatabase[userId].Encounters[placeId] then
        PlayerDatabase[userId].Encounters[placeId] = {
            PlaceName = placeName,
            Timestamps = {}
        }
    end
    
    table.insert(PlayerDatabase[userId].Encounters[placeId].Timestamps, timestamp)
end

function StartPlayerDBScanner()
    if not PlayerDB_Enabled then return end
    
    task.spawn(function()
        while PlayerDB_Enabled do
            for _, player in ipairs(Players:GetPlayers()) do
                DB_AddEncounter(player)
            end
            DB_Save()
            task.wait(60) -- Сканировать раз в минуту
        end
    end)
end

--[[
    Модуль: High Ping Notifier
]]
function StartPingNotifier()
    if not HighPingNotifier_Enabled then return end
    local notified = false
    task.spawn(function()
        while HighPingNotifier_Enabled do
            local ping = Players.LocalPlayer:GetNetworkPing() * 1000
            if ping > HighPingNotifier_Threshold and not notified then
                notify("Network Warning", "Your ping is high: " .. math.round(ping) .. "ms")
                notified = true
            elseif ping <= HighPingNotifier_Threshold and notified then
                notified = false
            end
            task.wait(5)
        end
...
-- ##################################################################
-- #            GODHAND - Модуль: Профиль Игрока (Profile)          #
-- #                    (ПРЯМОЙ КЛОН РАБОЧЕГО КОДА)                 #
-- ##################################################################
...
addcmd('profile', {'prof', 'whois', 'pinfo'}, function(args, speaker)
    ...
    -- [+] Уничтожаем старое окно, если оно есть, чтобы не было дубликатов
    if isProfileWindowOpen and isProfileWindowOpen:IsA("Frame") then
        isProfileWindowOpen:Destroy()
    end
    ...
    -- [+] GODHAND: Создаем кнопки
    local gotoButton = Instance.new("TextButton")
    local viewButton = Instance.new("TextButton")
    local linkButton = Instance.new("TextButton")

    isProfileWindowOpen = FRAME -- [+] Сохраняем ссылку на окно
    ...
    background.BorderSizePixel = 0
    background.Position = UDim2.new(0, 0, 1, 0)
    -- [+] Увеличена высота для кнопок
    background.Size = UDim2.new(0, 300, 0, 240) 
    ...
    PlayerImage.Position = UDim2.new(0.5, -30, 0, 10)
    ...
    -- [+] GODHAND: Настройка и размещение кнопок
    local function setButtonProperties(button, text, xPos)
        button.Parent = background
        button.BackgroundColor3 = currentShade2
        button.BorderSizePixel = 0
        button.Position = UDim2.new(0, xPos, 0, 200)
    ...
    setButtonProperties(gotoButton, "Goto", 5)
    setButtonProperties(viewButton, "View", 105)
    setButtonProperties(linkButton, "Link", 205)
    
    -- [+] Логика для кнопки View/Unview
    if viewing == targetPlayer then
        viewButton.Text = "Unview"
    end
    
    gotoButton.MouseButton1Click:Connect(function()
        execCmd("goto " .. targetPlayer.Name)
    end)
    
    viewButton.MouseButton1Click:Connect(function()
        if viewButton.Text == "View" then
            execCmd("view " .. targetPlayer.Name)
            viewButton.Text = "Unview"
        else
            execCmd("unview")
    ...
    linkButton.MouseButton1Click:Connect(function()
        local url = "https://www.roblox.com/users/" .. targetPlayer.UserId .. "/profile"
        toClipboard(url)
    ...
    task.spawn(function()
        local success, result = pcall(function()
            ...
            return string.sub(info.created, 1, 10)
        end)
    ...
    Exit.MouseButton1Click:Connect(function()
        ...
        task.wait(0.3)
        FRAME:Destroy()
        isProfileWindowOpen = nil -- [+] Сбрасываем ссылку
    end)
end)
